...
export function AccountImportDialog({ disabled, onImported }: AccountImportDialogProps) {
  const t = useTranslations("accountImport");
  ...
  const submitTokens = async (tokens: string[], successText?: string, accountPayloads: AccountImportPayload[] = []) => {
    ...
    try {
      ...
      if ((data.errors?.length ?? 0) > 0) {
        ...
        toast.error(
          `${successText ?? t("importComplete")}，新增 ${data.added ?? 0} 个，已刷新 ${data.refreshed ?? 0} 个，失败 ${data.errors?.length ?? 0} 个${firstError ? `，首个错误：${firstError}` : ""}`,
        );
      } else {
        toast.success(
          `${successText ?? t("importComplete")}，新增 ${data.added ?? 0} 个，跳过 ${data.skipped ?? 0} 个重复项，已自动刷新账号信息`,
        );
      }
    } catch (error) {
  ...
  const handleImportTokenText = async () => {
    await submitTokens(splitTokens(tokenInput), t("importComplete"));
  };
  ...
  const handleStartOAuth = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t("authStartFailed");
      toast.error(message);
    } finally {
      setOauthStarting(false);
    }
  ...
  const handleFinishOAuth = async () => {
    if (!oauthSession) {
      toast.error(t("openAuthPage"));
      return;
    }
    const trimmed = oauthCallbackInput.trim();
    if (!trimmed) {
      toast.error(t("finishImport"));
      return;
    }
    ...
    try {
      ...
      if ((data.errors?.length ?? 0) > 0) {
        const firstError = data.errors?.[0]?.error;
        toast.error(
          `${t("importComplete")}，新增 ${data.added ?? 0} 个，已刷新 ${data.refreshed ?? 0} 个，失败 ${data.errors?.length ?? 0} 个${firstError ? `，首个错误：${firstError}` : ""}`,
        );
      } else {
        toast.success(
          `${t("importComplete")}，新增 ${data.added ?? 0} 个，跳过 ${data.skipped ?? 0} 个重复项，已自动刷新账号信息`,
        );
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : t("importFailed");
  ...
  const handleImportSessionJson = async () => {
    ...
    try {
      ...
      await submitTokens([token], t("importComplete"));
    } catch (error) {
      const message = error instanceof Error ? error.message : t("importFailed");
      toast.error(message);
    }
  ...
  const handleImportCodexAuthJson = async () => {
    ...
    try {
      ...
      await submitTokens([account.access_token], t("importComplete"), [account]);
    } catch (error) {
      const message = error instanceof Error ? error.message : t("importFailed");
      toast.error(message);
    }
  ...
  const renderMethodBody = () => {
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-stone-400">{tokenCount} {t("importToken")}</span>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-stone-700">{t("importToken")}</label>
            <Textarea
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-1">
              <div className="text-sm font-medium text-stone-800">{t("importFromTxt")}</div>
              <div className="text-sm leading-6 text-stone-500">{t("importTokenDesc")}</div>
              </div>
              <Button
                type="button"
                variant="outline"
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            <div className="font-medium">{t("riskWarning")}</div>
            <div>
              {t("sessionJson")}
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium text-stone-700">{t("sessionJson")}</label>
            <Textarea
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-sm leading-6 text-stone-600 space-y-2">
            <div className="font-medium text-stone-800">{t("oauthLoginDesc")}</div>
            <ol className="list-decimal pl-5 space-y-1">
              <li>{t("oauthLoginDesc")}</li>
              <li>{t("openAuthPage")}</li>
              <li>{t("finishImport")}</li>
              <li>{tc("confirm")}</li>
            </ol>
          </div>
          <div className="space-y-2">
          ...
          {!oauthSession ? (
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-stone-700">{t("finishImport")}</label>
                <Textarea
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-5">
            <div className="space-y-2">
              <div className="text-sm font-medium text-stone-800">{t("selectJson")}</div>
          ...
          {pendingAccountJsonImport ? (
            <div className="rounded-2xl border border-stone-200 bg-white p-4 text-sm leading-6 text-stone-600">
              {t("importJson")} {pendingAccountJsonImport.parsedAccountCount} {t("importComplete")}
    ...
    if (method === "codex-auth") {
      return (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-stone-700">{t("importCodex")}</label>
            <Textarea
    ...
    return (
      <div className="space-y-3">
        ...
        <MethodCard
          title={t("importToken")}
          description={t("importTokenDesc")}
          icon={KeyRound}
          onClick={() => setMethod("token")}
        />
        <MethodCard
          title={t("importSession")}
          description={t("importSessionDesc")}
          icon={FileJson}
          onClick={() => setMethod("session")}
        />
        <MethodCard
          title={t("importCodex")}
          description={t("importCodexDesc")}
          icon={FileJson}
          onClick={() => setMethod("codex-auth")}
        />
        <MethodCard
          title={t("importJson")}
          description={t("importJsonDesc")}
          icon={Files}
          onClick={() => setMethod("account-json")}
        />
        <MethodCard
          title={t("importFromCPA")}
          description={t("importFromCPADesc")}
          icon={Files}
          onClick={() => {
            setOpen(false);
            resetState();
        ...
        />
        <MethodCard
          title={t("importFromSub2API")}
          description={t("importFromSub2APIDesc")}
          icon={ServerCog}
          onClick={() => {
            setOpen(false);
            resetState();
    ...
    <>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{t("confirmImportTitle")}</DialogTitle>
            <DialogDescription className="text-sm leading-6">
              {pendingAccountJsonImport
                ? `${t("confirmImportTitle")} ${pendingAccountJsonImport.parsedAccountCount} ${t("importComplete")}`
                : t("importFailed")}
              {pendingAccountJsonImport?.errorCount
                ? ` ${pendingAccountJsonImport.errorCount} ${t("importFailed")}`
                : ""}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="pt-2">
