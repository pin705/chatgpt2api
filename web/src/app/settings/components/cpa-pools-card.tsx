      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{t('connectionCount', { count: pools.length })}</Badge> : null}
                    ...
                    <div className="space-y-2 rounded-xl bg-stone-50 px-3 py-3">
                      ...
                      <div className="rounded-lg border border-stone-200 bg-white px-3 py-3">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                              <div className="text-sm font-medium text-stone-700">
                               {t('status')} {importJob.status}，{t('processed')} {importJob.completed}/{importJob.total}
                            </div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                          <span>{t('new')} {importJob.added}</span>
                          <span>{t('skipped')} {importJob.skipped}</span>
                          <span>{t('refresh')} {importJob.refreshed}</span>
                          <span>{t('failed')} {importJob.failed}</span>
                        </div>
                      </div>
                    </div>
                  ) : null}
