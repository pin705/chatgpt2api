...
function ImageManagerContent() {
  ...
  const loadImages = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("loadImagesFailed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSetTags = async (item: ManagedImage, tags: string[]) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("setTagFailed"));
    }
  };
  ...
  const handleDeleteTag = async (tag: string) => {
    try {
      ...
      toast.success(t("tagDeleted", { name: tag, count: result.removed_from }));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("deleteTagFailed"));
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("deletedCount", { count: data.removed }));
      setDeleteMode(null);
      setSelectedPaths([]);
      await loadImages();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("deleteImageFailed"));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      ...
      toast.success(t("downloaded", { count: paths.length }));
    } catch (error) {
    ...
    <section className="space-y-5">
        ...
        {storage ? (
          <>
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">{t("diskTotal")}</div>
            ...
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">{t("diskFree")}</div>
            ...
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">{t("imageCount")}</div>
              <div className="text-lg font-bold text-stone-800">{storage.image_count}</div>
            </div>
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">{t("imageUsage")}</div>
            ...
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3 col-span-2 flex items-center gap-2 flex-wrap">
              <span className="text-xs text-stone-500 w-full">{t("quickActions")}</span>
              <Button size="sm" variant="outline" className="h-7 text-xs" disabled={storageLoading} onClick={() => { void loadStorage(); }}>
                <RefreshCw className={`size-3 mr-1 ${storageLoading ? "animate-spin" : ""}`} />{t("refresh")}
              </Button>
              <Button size="sm" variant="outline" className="h-7 text-xs"
                onClick={async () => {
                  try { const r = await compressAllImages(); setCompressResult(t("compressed", { size: r.saved_mb })); void loadStorage(); }
                  catch { setCompressResult(t("compressFailed")); }
                }}>
                🗜️ {t("compress")}
              </Button>
              ...
              <form onSubmit={async (e) => { e.preventDefault();
                try {
                  ...
                  toast.success(`${t("deletedCount", { count: r.removed })}，释放 ${r.freed_mb ?? 0}MB`);
                  void loadStorage();
                } catch { toast.error(t("cleanFailed")); }
          ...
          <div className="rounded-xl border border-stone-200 bg-white/80 p-3 col-span-full text-center text-sm text-stone-400">
            {storageLoading ? t("loadingStorage") : t("storageLoadFailed")}
          </div>
        )}
      </div>
      ...
      <Dialog open={deleteMode === "byDate"} onOpenChange={() => setDeleteMode(null)}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader><DialogTitle>{t("deleteByDateTitle")}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <label className="text-sm text-stone-600 shrink-0">{t("deleteImage")}</label>
              <Input className="h-9 text-sm" type="date" value={deleteStartDate} onChange={(e) => setDeleteStartDate(e.target.value)} />
              <span className="text-sm text-stone-400">{t("before")}</span>
            </div>
            <p className="text-xs text-stone-500">{t("deleteByDateWarning")}</p>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDeleteMode(null)}>{t("cancel")}</Button>
            <Button variant="destructive" disabled={!deleteStartDate || isDeleting}
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success(t("deletedCount", { count: r.removed }));
                  setDeleteMode(null);
                  void loadStorage();
                  void loadImages();
        ...
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-100 px-5 py-4">
            <div className="flex flex-wrap items-center gap-3 text-sm text-stone-600">
              ...
              {t("totalImages", { count: filteredItems.length })}
                    ...
                    <Popover open={tagEditTarget?.rel === item.rel} onOpenChange={(open) => { setTagEditTarget(open ? item : null); setTagInput(""); }}>
                      ...
                      <PopoverContent align="start" className="w-56 p-2">
                        <div className="space-y-2">
                          <div className="text-xs font-medium text-stone-500">{t("addTag")}</div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-stone-100 px-4 py-3 text-sm text-stone-500">
            <span>{t("pageInfo", { current: safePage, total: pageCount, count: filteredItems.length })}</span>
          ...
          {!isLoading && filteredItems.length === 0 ? <div className="px-6 py-14 text-center text-sm text-stone-500">{t("noImages")}</div> : null}
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogVisible} onOpenChange={(open) => { if (!open) closeDialog(); }}>
        <DialogContent className="max-w-sm overflow-hidden rounded-2xl">
          <DialogHeader>
            <DialogTitle className="pr-8">{t("confirmDelete")}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={deleteMode === "selected" || deleteMode === "filtered"} onOpenChange={(open) => (!open ? setDeleteMode(null) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{deleteMode === "filtered" ? t("deleteByDateTitle") : t("deleteSelectedTitle")}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-stone-600">
      ...
      <Dialog open={Boolean(tagDeleteTarget)} onOpenChange={(open) => { if (!open) setTagDeleteTarget(null); }}>
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader>
            ...
            <p className="text-sm text-stone-600">
              {t("confirmDeleteTag", { name: tagDeleteTarget || "" })}
          </p>
          <DialogFooter>
