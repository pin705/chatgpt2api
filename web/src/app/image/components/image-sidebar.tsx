...
export function ImageSidebar({
  ...
  const t = useTranslations("imageSidebar");
      ...
      <div className="flex h-full min-h-0 flex-col gap-2 py-1 sm:gap-3 sm:py-2">
        ...
        >
          {isLoadingHistory ? (
            ...
            <div className="px-2 py-3 text-sm leading-6 text-stone-500">{t("noHistory")}</div>
          ) : (
                ...
                >
                  ...
                  >
                    ...
                    <div className={cn("mt-1 text-xs", active ? "text-stone-500" : "text-stone-400")}>
                      {t("turnCount", { count: conversation.turns.length })} · {formatConversationTime(conversation.updatedAt)}
                    </div>
                    {stats.running > 0 || stats.queued > 0 ? (
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                        {stats.running > 0 ? (
                          <span className="rounded-full bg-blue-50 px-2 py-1 text-blue-600">{t("processing", { count: stats.running })}</span>
                        ) : null}
                        {stats.queued > 0 ? (
                          <span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">{t("queuing", { count: stats.queued })}</span>
                        ) : null}
                      </div>
                    ) : null}
                  </button>
